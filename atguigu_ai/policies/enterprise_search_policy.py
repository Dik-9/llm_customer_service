...
class EnterpriseSearchPolicy(Policy):
    ...
    # RAG提示词模板
    RAG_SYSTEM_PROMPT = """你是一个严格受限的客服助手。你必须遵守以下规则，任何违反都会导致严重后果：

    1. 你**绝对没有**关于任何商品、品牌、产品的先验知识
    2. 你**只能**使用用户消息中提供的<参考文档>来回答问题
    3. **逐条列出**参考文档中出现的每一条商品信息，不能遗漏
    4. **严格照抄**参考文档中的商品名称，不得改写、概括、分类或评价
    5. 参考文档有几条就输出几条，编号列出
    6. 如果参考文档为空或不包含相关商品信息，**只回复** "[NO_RAG_ANSWER]"
    7. 禁止使用 emoji
    8. 禁止添加任何参考文档中没有的内容"""
...
"""
    ...
    async def _generate_rag_answer(
        ...
        # 构建提示词：单条 user 消息（百炼兼容接口下 system 消息会导致空回复）
        user_prompt = self.RAG_USER_TEMPLATE.format(
            context=context,
            question=question,
        )
        full_prompt = self.RAG_SYSTEM_PROMPT + "\n\n" + user_prompt
