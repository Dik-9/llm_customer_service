...
class LangChainClient(LLMClient):
    ...
    def _create_qwen_llm(self):
        """创建ChatTongyi实例（阿里云百炼/DashScope通义千问）
        ...
        # 支持百炼平台业务空间专属 URL
        # DashScope SDK 通过 dashscope.base_http_api_url 配置 API 地址
        if self.api_base:
            import dashscope
            dashscope.base_http_api_url = self.api_base
            _logger.warning(f"[LLM] 设置 dashscope.base_http_api_url: {self.api_base}")
