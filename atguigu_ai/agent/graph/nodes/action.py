...
async def _execute_action(
    ...
    - MCP 启用且 action 在映射表 → MCPExecutable（失败自动降级本地直调）；
    - 否则走 get_action 本地直调；本地未找到时返回 None（由调用方按基线 not-found 处理）。

    MCP 关闭（tool_registry 为 None 或 has_mcp_route 全 False）时全部走本地路径，
    与基线 get_action(action_name).run(...) 行为等价（SPEC §6.4）。
    """
    use_mcp = tool_registry is not None and tool_registry.has_mcp_route(action_name)
    if use_mcp:
        executable = tool_registry.get(action_name)
        return await executable.run(tracker, domain, **kwargs)
...
async def action_node(state: "MessageProcessingState") -> Dict[str, Any]:
    ...
    2. 通过统一执行入口查找动作（本地 Action 或 MCP 工具）
    3. 执行动作
    ...
    try:
        ...
        # 如果是闲聊动作，传递 LLM 配置（闲聊动作不在 MCP 映射表，必走本地路径）
        ...
        # 执行动作（统一执行入口，SPEC §6.2）
        # MCP 启用且 action 在映射表 → MCPExecutable（失败自动降级本地）；
        # 否则 get_action 本地直调；本地未找到时返回 None（等价基线 not-found）
        result = await _execute_action(action_name, tracker, domain, kwargs, tool_registry)
        if result is None:
        ...
        if (not result.responses
            ...
            # fallback 同样走统一执行入口（fallback 可能是 MCP 映射的 action_ask_xxx）
            fallback_result = await _execute_action(
                fallback_action_name, tracker, domain, kwargs, tool_registry
            )
