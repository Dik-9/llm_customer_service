...
class GraphRAG(InformationRetrieval):
    ...
    def connect(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # 4、配置 LLM（用于 Cypher 生成/校验/校正等复杂任务）
        # 优先从 connect() 传入的 config 中读取 LLM 配置（由 config.yml + endpoints.yml 统一管理）
        llm_config = config.get("llm", {})
        model_name = llm_config.get("model") or os.getenv("DASHSCOPE_MODEL") or "qwen3.6-plus"
        model_api_key = llm_config.get("api_key") or os.getenv("DASHSCOPE_API_KEY")
        model_api_base = llm_config.get("api_base")
        if not model_api_base:
            workspace_id = os.getenv("DASHSCOPE_WORKSPACE_ID", "llm-xcmc3bcdk6yz5n2b")
            model_api_base = f"https://{workspace_id}.cn-beijing.maas.aliyuncs.com/compatible-mode/v1"
        model_temperature = llm_config.get("temperature", 0)
        # GraphRAG 内部 LLM 调用（Cypher 生成/验证/校正）prompt 包含完整 Neo4j schema，需要较大输出空间
        # 至少 8192 token，config 中配的值如果更大则用 config 的
        model_max_tokens = max(llm_config.get("max_tokens", 8192), 8192)
    ...
    async def route_label(self, query):
        ...
        # 2、调用LLM，获得输出结果
        # 直接使用普通调用方式 + JSON 解析（百炼兼容接口的 structured output 行为不一致）
        llm_output = await self.llm.ainvoke(prompt)
        json_str = self._extract_json_from_text(llm_output.content)
        logger.debug(f"GraphRAG route_label raw content: {repr(llm_output.content)[:300]}")
        logger.debug(f"GraphRAG route_label extracted json: {json_str[:300]}")
        try:
            data = json.loads(json_str)
            if isinstance(data, list):
                outputs = [RouteItem(**item) for item in data]
            elif isinstance(data, dict) and "outputs" in data:
                outputs = [RouteItem(**item) for item in data["outputs"]]
            else:
                outputs = []
        except (json.JSONDecodeError, TypeError) as e:
            logger.warning(f"GraphRAG route_label JSON 解析失败: {e}")
            outputs = []
    ...
    async def search(
        ...
        # 获取聊天历史（包含最新用户消息）
        chat_history = (get_chat_history(tracker_state, user_id) or query) if tracker_state else query
        # 获取入口节点标签
